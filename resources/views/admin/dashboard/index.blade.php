...
@section('content')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
            ...
            {{-- Card: Revenue --}}
            <div class="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 p-5 relative overflow-hidden">
                ...
                <div class="flex items-start justify-between relative z-10">
                    <div>
                        ...
                        <div class="flex items-center gap-1 mt-2">
                            ...
                            @else
                                <span class="inline-flex items-center text-xs font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
                    ...
                    <div class="p-3 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-xl shadow-lg shadow-emerald-200 group-hover:scale-110 transition-transform duration-300">
            ...
            <div class="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 p-5 relative overflow-hidden">
                ...
                <div class="flex items-start justify-between relative z-10">
                    ...
                    <div class="p-3 bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl shadow-lg shadow-blue-200 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 p-5 relative overflow-hidden">
                ...
                <div class="flex items-start justify-between relative z-10">
                    <div>
                        ...
                        <div class="flex items-center gap-1 mt-2">
                            @if($lowStockCount > 0)
                                <span class="inline-flex items-center text-xs font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">
                                    <svg class="w-3 h-3 mr-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
                    ...
                    <div class="p-3 bg-gradient-to-br from-violet-400 to-violet-600 rounded-xl shadow-lg shadow-violet-200 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {{-- Card: Clients --}}
            <div class="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 p-5 relative overflow-hidden">
                <div class="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-rose-50 to-transparent rounded-bl-full opacity-80"></div>
                <div class="flex items-start justify-between relative z-10">
                    <div>
                    ...
                    <div class="p-3 bg-gradient-to-br from-rose-400 to-rose-600 rounded-xl shadow-lg shadow-rose-200 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="px-5 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center flex-shrink-0">
                                        <span class="text-xs font-bold text-white">{{ substr($order->client->name ?? $order->client_name ?? 'N', 0, 1) }}</span>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-800">{{ $order->order_number ?? '#'.$order->id }}</p>
                                        <p class="text-xs text-gray-400">{{ $order->client->name ?? $order->client_name ?? 'Cliente' }} · {{ $order->created_at->diffForHumans() }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    ...
                                    @endphp
                                    <span class="inline-block mt-0.5 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase {{ $color }}">{{ $order->status ?? 'N/A' }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    ...
                    <div class="flex items-end justify-between">
                        <div>
                            <p class="text-2xl font-extrabold text-gray-800">Bs {{ number_format($pendingReceivables, 2) }}</p>
                            <p class="text-xs text-gray-400 mt-1">{{ $totalReceivablesCount }} {{ $totalReceivablesCount == 1 ? 'cuenta pendiente' : 'cuentas pendientes' }}</p>
                        </div>
                            ...
                            @foreach($ordersByStatus as $status => $count)
                                ...
                                <div>
                                    ...
                                    <div class="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                                        <div class="{{ $barColor }} h-2 rounded-full transition-all duration-700" style="width: {{ $percentage }}%"></div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl border border-amber-200 shadow-sm p-5 mb-8">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-amber-600">{{ $lowStockCount }} {{ $lowStockCount == 1 ? 'producto necesita' : 'productos necesitan' }} reposición</p>
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                    @foreach($lowStockProducts as $product)
                        <div class="bg-white rounded-xl border border-amber-100 p-3 flex items-center gap-3 hover:shadow-md transition-shadow">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-amber-600 font-bold">Stock: {{ number_format($product->inventory->stock ?? 0, 0) }} / Mín: {{ number_format($product->inventory->minimum_stock ?? 0, 0) }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
