    ...
    <div>
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
            ...
            <div class="flex space-x-2">
                {{-- Botón opcional para ver historial de movimientos --}}
                <button
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="bg-white p-6 rounded-xl shadow border-l-4 border-red-500">
                ...
                <p class="text-2xl font-bold text-red-600">{{ $inventories->where('stock', '<=', 0)->count() }}</p>
            </div>
            <div class="bg-white p-6 rounded-xl shadow border-l-4 border-orange-500">
                ...
                <p class="text-2xl font-bold text-orange-600">
                    {{ $inventories->filter(fn($i) => $i->stock <= $i->minimum_stock && $i->stock > 0)->count() }}
                </p>
            </div>
            <div class="bg-white p-6 rounded-xl shadow border-l-4 border-green-500">
                <h2 class="text-gray-500 text-sm font-medium uppercase">Total Unidades en Almacén</h2>
                <p class="text-2xl font-bold text-green-600">{{ number_format($inventories->sum('stock'), 0) }}</p>
            </div>
        </div>
                        ...
                        @forelse($inventories as $inventory)
                            <tr x-data="{ openAdjust: false }" class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-orange-600 font-medium">
                                    {{ number_format($inventory->reserved_stock, 0) }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                    {{-- Botón para Ajuste Manual (Mermas, robos, errores de conteo) --}}
                                            ...
                                            <div class="flex items-center justify-center min-h-screen px-4">
                                                ...
                                                <div
                                                    ...
                                                    <form action="{{ route('admin.inventories.update', $inventory->id) }}"
                                                        ...
                                                        @method('PUT')
                                                        <div class="p-6">
                                                            <h3 class="text-xl font-bold text-gray-900 mb-4 text-left">
                                                                Ajuste de Stock: {{ $inventory->product->name }}</h3>
                                                            ...
                                                            <div class="space-y-4 text-left">
                                                                <div>
                                                                    ...
                                                                    <select name="adjustment_type"
                                                                        ...
                                                                        <option value="addition">➕ Sumar al inventario
                                                                            (Entrada)
                                                                        </option>
                                                                        <option value="subtraction">➖ Restar del inventario
                                                                            (Salida/Merma)</option>
                                                                    </select>
                                                                </div>
                                                                <div>
                                                                ...
                                                                <div>
                                                                    <label
                                                                        ...
                                                                        del Ajuste</label>
                                                                    <textarea name="reason" rows="2" class="mt-1 w-full rounded-lg border-gray-300"
                                                                        placeholder="Ej: Producto dañado, error de conteo inicial..." required></textarea>
                                                                </div>
                                                            </div>
                                                        </div>
