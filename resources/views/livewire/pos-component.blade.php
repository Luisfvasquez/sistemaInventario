<div class="grid grid-cols-1 lg:grid-cols-12 gap-6 relative">
    ...
    @if (session()->has('error'))
        <div class="lg:col-span-12 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded shadow-sm font-bold">
            {{ session('error') }}
        </div>
    @endif
                            ...
                            @foreach ($prod->bulks as $bulk)
                                <li wire:click="addToCart({{ $prod->id }}, {{ $bulk->id }})"
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500">Stock: {{ $prod->inventory->stock ?? 0 }} Und
                                            base</div>
                                    </div>
        ...
        <div class="bg-white p-5 rounded-xl shadow-sm border-t-4 border-emerald-500">
            <div class="flex justify-between items-center mb-3">
                ...
                <button wire:click="addPaymentLine"
                    class="text-xs bg-gray-200 px-2 py-1 rounded hover:bg-gray-300 font-bold">+ Dividir Pago</button>
            </div>
        ...
        <div class="bg-gray-900 text-white p-6 rounded-xl shadow-xl">
            <div class="space-y-2">
                <div class="flex justify-between items-center text-gray-300">
                    ...
                    <span class="font-black text-xl">Bs. {{ number_format(collect($cart)->sum('subtotal'), 2) }}</span>
                </div>
                <div class="flex justify-between items-center text-gray-300">
                    <span class="text-sm">Entregado en Caja:</span>
                    <span class="font-bold text-green-400 text-lg">Bs. {{ number_format($amount_received, 2) }}</span>
                </div>

                {{-- Detección de FIADO (Deuda) --}}
                @if ($amount_pending > 0)
                    <div class="flex justify-between items-center bg-red-500/20 p-2 rounded border border-red-500 mt-2">
                        <span class="text-red-300 text-xs font-bold uppercase tracking-wider">Monto Fiado (Deuda)</span>
                ...
                @elseif($amount_pending < 0)
                    <div
                        ...
                        <span class="text-blue-300 text-xs font-bold uppercase tracking-wider">Cambio / Vuelto</span>
                        <span class="font-black text-blue-400">Bs. {{ number_format(abs($amount_pending), 2) }}</span>
                    </div>
                @endif
